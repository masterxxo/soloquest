...
async function onSubmit() {
  ...
  if (error) {
    errorMsg.value = error.message ?? 'Could not create your account.';
    return;
  }
...
<template>
  <section class="dl-row-in corner-cut w-full max-w-[380px] border border-dl-grid-line bg-dl-surface">
    ...
    <header class="flex items-center gap-2 border-b border-dl-band-line px-5 py-3">
      ...
      <span class="font-dl-mono text-dl-label uppercase tracking-wide text-dl-ink-muted">Register</span>
    </header>

    <div class="px-5 py-6 md:px-6">
      <h1 class="font-dl-display text-dl-title font-semibold leading-tight text-dl-ink">Create account</h1>
      <p class="mt-1 text-dl-body text-dl-ink-muted">Register to begin your ascent.</p>
      ...
      <form @submit.prevent="onSubmit" class="mt-6 flex flex-col gap-4">
        ...
        <label class="flex flex-col gap-1.5">
          ...
          <input v-model="name" type="text" required autocomplete="name" class="dl-focus-inset border border-dl-grid-line bg-dl-surface px-3 py-2 text-dl-body text-dl-ink outline-none placeholder:text-dl-ink-faint" />
        </label>
        <label class="flex flex-col gap-1.5">
        ...
        <label class="flex flex-col gap-1.5">
          ...
          <input v-model="password" type="password" required autocomplete="new-password" class="dl-focus-inset border border-dl-grid-line bg-dl-surface px-3 py-2 text-dl-body text-dl-ink outline-none placeholder:text-dl-ink-faint" />
        </label>
      ...
      <p class="mt-6 text-center text-dl-body text-dl-ink-muted">
        ...
        <NuxtLink to="/login" class="dl-focus-inset font-medium text-dl-violet hover:text-dl-violet-hot hover:underline">Sign in</NuxtLink>
      </p>
    </div>
  </section>
</template>
